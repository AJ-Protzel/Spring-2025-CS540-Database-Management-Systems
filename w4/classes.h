...
public:
    ...
    void write_into_data_file(ostream& out) const {
        ...
        out.write(bio, sizeof(bio));  // Write the bio
    ...
    void read_from_data_file(istream& in) {
        ...
        /***TO_DO***/ // do the same thing for bio and manager-id
        in.read(bio, sizeof(bio));  // Read the bio
...
public:
    ...
    void createFromFile(const string& csvFilename) {
        ...
        while (getline(csvFile, line)) {
            ...
            getline(ss, name, ',');
            getline(ss, bio, ',');
            getline(ss, manager_id_str, ',');

            id = stoi(id_str);
            manager_id = stoi(manager_id_str);
    ...
    void findAndPrintEmployee(int searchId) {
        ...
        while (data_file.peek() != EOF) {  // Check if end-of-file is reached
            emp.read_from_data_file(data_file);  // Read the Employee record
            if (emp.id == searchId) {  // Check if the ID matches
                emp.print();  // Print the Employee details
                return;  // Exit the function
            }
        }
       
        // Print not found message if no match
        cout << "Employee with ID " << searchId << " not found." << endl;  // Print not found message if no match
